    ...
    <div class="main_area dashboard pb-3 pt-3">
        <div class="container">
            <div class="dashboard_area">
             ...
             $user_last_activity =  $value->qt_entry_date;
             ...
             $user_photo=$value->user_photo;
                                    ...
                                    <li class="offerer">
                                      ...
                                      <?php }?>
                                        ...
                                        <div class="user-name">
                                           <h4><a href="<?php echo base_url(); ?>quote/message_list?ads=<?=base64_encode($ads);?>&user=<?=base64_encode($value->id);?>"><?=$value->name;?></a></h4>
                                           <h5><!-- <?=$status;?> --><?php $msg_no=unreadMessage($value->id,$value->lbcontactId,$this->session->userdata('front_sess')['userid']);
...
<?php 
$tm_adid=base64_decode($this->input->get('ads'));
$with=base64_decode($this->input->get('user'));
...
?>        
        <div class="chat-about">
          ...
          <div class="chat-num-messages">total <?=totalChatMessage($user,$ads,$this->session->userdata('front_sess')['userid']);?> messages</div>
    ...
    cursor: pointer;
    line-height: 3px;
"><a href="<?php echo base_url(); ?>quote/clear_chat?adid=<?php echo $tm_adid; ?>&with=<?php echo $with; ?>&loginid=<?php echo $user_id; ?>" onclick="return confirm('Are you sure to clear?')">Clear chat &nbsp;<i class="fa fa-trash"></i></a></div>
        </div>
...
if($user_id==$owner && $value->adelete==0){
                                        if($value->qt_userid==$this->session->userdata('front_sess')['userid'])
                              {
                               $user_name = 'You';
...
<?php }elseif($user_id!=$owner && $value->bdelete==0){ ?>
