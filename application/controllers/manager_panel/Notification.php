...
class Notification extends CI_Controller {
        ...
        function __construct()
    {
        ...
        $this->load->model('manager_panel/General_staff_model');
    ...
    {
       $userid=$this->session->userdata('logged_in_mng')['manager_id'];
        $data['datatable']=$this->General_staff_model->show_data_id_join('notification','user_id','user_table','id','','');
        ...
        $this->load->view('manager_panel/notification');
...
$userid=$this->session->userdata('logged_in_stf')['staff_id'];
       ...
       $this->form_validation->set_rules('description', 'description', 'required');
        if ($this->form_validation->run() == FALSE) 
        {
           ...
           redirect('manager_panel/notification/'); 
            //echo 0;
        }else{
