...
function quote_list($offset=0){ 
      ...
      if(count($_GET) > 0){
        $config['first_url'] = site_url("apanel/quote/quote_list".'?'.http_build_query($_GET));
      }

        $config["base_url"]=site_url("apanel/quote/quote_list"); 
      ...
      //print_r($data);exit();
        ...
        $this->load->view('apanel/header',compact('quote_list','page_link','title'));

        $this->load->view('apanel/sp_quote_list');
   ...
   $data['user']=$chat_user;
     $data['title']="Message list";
        ...
        $this->load->view('apanel/sp_quote_message_list');
...
//--------------------  pagination ---------------//
    ...
    function manager_list(){ 
      //print_r($data);exit();
        ...
        $data['user']=$this->General_model->show_data_id('staff_table','manager_staff','user_type','get','');

        $this->load->view('apanel/header',$data);

        $this->load->view('apanel/manager_list');
 ...
 public function post_chat()
  {
   ...
   $user_id=$this->session->userdata('logged_in')['admin_id'];
