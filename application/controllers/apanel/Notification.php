...
// 	{
// 		$data['datatable']=$this->General_model->show_data_id_join('notification','user_id','user_table','id','','');
...
// 		$this->load->view('apanel/notification');
...
if(!$this->session->userdata('is_logged_in')==1)
    ...
    public function notice_post()
    {
$userid=$this->session->userdata('logged_in')['admin_id'];
        $this->form_validation->set_rules('user_id', 'Customer', 'required');
       $this->form_validation->set_rules('description', 'description', 'required');
        if ($this->form_validation->run() == FALSE) 
        {
