<?php 
 class Model_users extends CI_Model{
    ...
    public function login($data) {
		$condition="email='".$data['username']."' and password='".md5($data['password'])."' and user_type='manager_staff' and status='Active' ";
		$this->db->select('*');
...
//$sql = $this->db->last_query();
	//return $sql;
		...
		$this->db->from('admin_details');
            ...
            $this->session->set_userdata('access_id_mng',$id[0]["id"]);
            //End Admin Access
            $session_data = array(
					'username'=>$id[0]["name"],
					'name'=>$id[0]["name"],
			...
			$this->session->set_userdata('logged_in_mng', $session_data);
