...
function allquote_data_list($limit,$offset){
    ...
    $query=$this->db->query("SELECT tq.*,ml.lbcontactId,ml.title,GROUP_CONCAT(DISTINCT qt_userid)as total_qt_userid FROM `table_quote` tq INNER JOIN module_lbcontacts ml on ml.lbcontactId=tq.qt_adsid  WHERE  (qt_adsuser!=`qt_userid`) AND qt_isdelete=0 group by `qt_adsid` ORDER BY qt_id DESC LIMIT $limit OFFSET $offset")->result();
